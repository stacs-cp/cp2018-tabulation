...
public class QuantifiedSum extends Quantifier
{
	...
	public PairASTNode getBoundsAST() {
	    ...
	    if(getChild(1) instanceof SimpleDomain) {
	        ...
	        ASTNode domsize=BinOp.makeBinOp("+", BinOp.makeBinOp("-", dombnds.e2, dombnds.e1), NumberConstant.make(1));
	        ...
	        // domsize is now size of base domain. 
	        ASTNode numslots=NumberConstant.make(1);  // upper bound on number of items in each matrix.
	        for(int i=3; i<getChild(1).numChildren(); i++) {
	            PairASTNode idxdombnds=getChild(1).getChild(i).getBoundsAST();
	            ASTNode idxdomsize=BinOp.makeBinOp("+", BinOp.makeBinOp("-", idxdombnds.e2, idxdombnds.e1), NumberConstant.make(1));
