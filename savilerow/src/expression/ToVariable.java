...
public class ToVariable extends ASTNodeC
{
    ...
    public ASTNode simplify() {
	    ...
	    if(CmdFlags.getMiniontrans() && false) {
	        ...
	        if(c0 instanceof Element || c0 instanceof ElementOne || c0 instanceof SafeElement || c0 instanceof SafeElementOne) {
	            if(c0.getChild(0).getCategory() == ASTNode.Constant && (getChild(1) instanceof Identifier)) {
	                ...
	                TransformMakeTable tmt=new TransformMakeTable(getChild(1).getModel());
	                
	                //  Look up in the cache for a long table. 
	                TransformMakeTable.RetPair ret=tmt.tryCacheNormalised(this, false);
	                ...
	                ASTNode a=tmt.normalise(this);
	                
	                ASTNode newTable = tmt.makeTableLong(a, Long.MAX_VALUE, Long.MAX_VALUE);
    ...
    {
        assert bool_context;
        if(getChild(0).isRelation() && !(getChild(0) instanceof SafeElement) && !(getChild(0) instanceof Element) && !(getChild(0) instanceof ElementOne) )
        {
            b.append("reify(");
