        ...
        for(ASTNode indexdom : indices)
        {
            ...
            if(indexdom.getCategory() ==ASTNode.Constant) {
                ...
                if(idxvals.get(idxvals.size()-1)-idxvals.get(0)+1 > idxvals.size()) {
                    ...
                    offsets.add(NumberConstant.make(-lowervalue+new_lb));
                    maps.add(null);
                ...
                ASTNode adjustment=BinOp.makeBinOp("+", new UnaryMinus(indexdom.getBoundsAST().e1), NumberConstant.make(new_lb));
	        ...
	        if(curnode.getChild(0) instanceof Identifier) {
                ...
                if(name.equals(matrixname)) {
                    ...
                    if(curnode instanceof MatrixDeref) {
                        return new NodeReplacement(new Tag(new MatrixDeref(curnode_children.get(0), new_indices)));
                    }
                    else {
                        return new NodeReplacement(new Tag(new SafeMatrixDeref(curnode_children.get(0), new_indices)));
                    }
                }
	        }
	    }
                    ...
                    for(int i=0; i<curnode_children.size()-1; i++)
                    {
                        ...
                        if(old_index instanceof Range)
                        {
                            for(int j=0; j<old_index.numChildren(); j++) {
                                if(offsets.get(i)!=null) {
                                    old_index.setChild(j, BinOp.makeBinOp("+", old_index.getChild(j), offsets.get(i)));
                                }
                                else {
