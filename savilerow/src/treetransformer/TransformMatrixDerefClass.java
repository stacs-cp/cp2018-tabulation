...
public class TransformMatrixDerefClass extends TreeTransformerBottomUpNoWrapper
{
	...
	protected ASTNode matrixDerefToElement(ASTNode arrayVariable)
	{
	    ...
	    ArrayList<ASTNode> indices = arrayVariable.getChildren(1);
	    ...
	    ASTNode matrixslice=new MatrixSlice(m, matrix, constant_indices);
	    ...
	    for(int i=0; i<arraydimensions2.length; i++) {
	        ...
	        arraydimensions2[i]=BinOp.makeBinOp("+", BinOp.makeBinOp("-", bnds.e2, bnds.e1), NumberConstant.make(1));
	    }
	    ...
	    for(int j=0; j<variable_indices_indices.size(); j++)
	    {
	        ...
	        ASTNode prod=NumberConstant.make(1);
