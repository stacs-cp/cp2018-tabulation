            ...
            if(ch.get(1) instanceof CompoundMatrix && ch.get(2) instanceof CompoundMatrix)
            {
                ...
                for(int i=1; i<values.size(); i++)   // From 1 now because of CompoundMatrix index
                {
                    ...
                    if(curnode instanceof AtMost) {
                        new_cons.add(new Tag(new AtMost(ch.get(0), CompoundMatrix.make(newoccs), CompoundMatrix.make(newvals))));
                    }
                    else
                    {
                        new_cons.add(new Tag(new AtLeast(ch.get(0), CompoundMatrix.make(newoccs), CompoundMatrix.make(newvals))));
                    }
                }
